...
async def create_check(amount, userid):
    ...
    if bal < amount:
        ...
        return
    headers = {"Crypto-Pay-API-Token": config.CRYPTOPAY_TOKEN}
    data = {"asset": "USDT", "amount": float(amount*1.1), "pin_to_user_id": userid}
...
<b>🛂 В случае проблем с зачислением средств на ваш счёт обратитесь к администратору (<a href="{config.OWNER_LINK}">*тык*</a>)</b>""", reply_markup=keyb, disable_web_page_preview=True)
        ...
        if call.from_user.id in config.ADMINS:
            ...
            for item in items:
                ...
                if status == None:
                    ...
                    btns.append(InlineKeyboardButton(f"{status} - {item['amount']} {item['asset']}", callback_data=f'checks:{check_id}'))
            keyb.add(*btns)
        else:
    ...
    elif call.data.startswith("checks:"):
        if call.from_user.id in config.ADMINS:
    ...
    elif call.data == 'cb_pay':
        user_id = call.from_user.id
        balance = get_balance(user_id)

        # Move the 'cb' declaration outside the 'elif' block
        cb = types.InlineKeyboardMarkup(row_width=2)
        check = await create_checkcb(balance, user_id) # Get the URL from create_checkcb

        # Now 'check' should be a string
        if isinstance(check, str): # Add a check for a valid string
            cb.add(InlineKeyboardButton(f"⚡️ Забрать {balance:.2f}$", url=check))
            await bot.send_message(chat_id=call.message.chat.id, text=f"<blockquote><b>⚡️ вот ваш вывод</b></blockquote>", reply_markup=cb)
        else:
            # Handle the case where 'check' is not a string (e.g., log the error)
            print(f"Error: 'check' is not a string: {check}")

        cursor.execute("UPDATE users SET cashback=0.0 WHERE us_id=?", (user_id,))
        conn.commit()
    elif call.data == 'ref_pay':
        user_id = call.from_user.id
        ...
        cursor.execute("UPDATE users SET ref_balance=0.0 WHERE us_id=?", (user_id,))
        conn.commit()
    else:
        await call.answer()

async def create_checkref(amount, userid):
    bal = get_cb_balance()
    bal = float(bal)
    amount = float(amount)

    # Separate InlineKeyboardMarkup objects for different messages
    keyb_user = types.InlineKeyboardMarkup().add(types.InlineKeyboardButton("💼 Перейти к пользователю", url=f"tg://user?id={userid}"))
    keyb_logs = InlineKeyboardMarkup(row_width=1).add(InlineKeyboardButton("Закрыть", callback_data='close'))  # Or other data for this button
    
    if bal < amount:
        try:
            await bot.send_message(userid, f"<b>[🔔] Вам пришло системное уведомление:</b>\n\n<b><blockquote>Ваша выплата ⌊ {amount}$ ⌉ будет зачислена вручную администратором!</blockquote></b>", reply_markup=keyb_user)
        except:
            pass
        await bot.send_message(config.LOGS_ID, f"<b>[🔔] Мало суммы в казне для выплаты!</b>\n\n<b><blockquote>Пользователь: {userid}\nСумма: {amount}$</blockquote></b>", reply_markup=keyb_logs)
        return
    headers = {"Crypto-Pay-API-Token": config.CRYPTOPAY_TOKEN}
    data = {"asset": "USDT", "amount": float(amount), "pin_to_user_id": userid}
    r = requests.get("https://pay.crypt.bot/api/createCheck", headers=headers, data=data).json()
    await bot.send_message(config.LOGS_ID, f"<b>[🧾] Создан чек!</b>\n\n<b>[💠] Сумма: {amount} USDT</b>\n<b>[🚀] Прикрепен за юзером: {userid}</b>", reply_markup=keyb_logs)
    return r["result"]["bot_check_url"]

def get_ref_balance(user_id):
 """
 Retrieves the referral balance for a given user ID from the database.

 Args:
  user_id: The ID of the user to retrieve the balance for.

 Returns:
  The referral balance for the user, or 0 if no balance is found.
 """

 try:
  # Connect to your SQLite database
  conn = sqlite3.connect('db.db') # Replace 'your_database.db' with your actual database file

  # Create a cursor object
  cursor = conn.cursor()

  # Execute the SQL query to get the referral balance
  cursor.execute("SELECT ref_balance FROM users WHERE us_id = ?", (user_id,))
  ...
  # Return the ref balance if found, otherwise return 0
  return result[0] if result else 0
 except Exception as e:
  print(f"Error retrieving ref balance: {e}")
  return 0

async def create_checkcb(amount, userid):
    bal = get_cb_balance()
    bal = float(bal)
    amount = float(amount)
...
def get_balance(user_id):
  with sqlite3.connect("db.db") as conn:
      cursor = conn.cursor()
      cursor.execute("SELECT cashback FROM users WHERE us_id=?", (user_id,))
...
def parse_message(message, message_obj):
    amount = float(message_obj.text.split("отправил(а) \U0001fa99")[1].split()[0].replace(',', "")) #получаем сумму ставки
    name = message_obj.text.split("отправил(а) \U0001fa99")[0] #получаем имя чела
    if message_obj.entities[0].user: #проверяем есть ли ссылка на чела
        user = message_obj.entities[0].user
        username = f"@{user.username}" if user.username else user.full_name
        name = user.full_name #снова получаем имя более надежным способом
        msg_text = message_obj.text.removeprefix(name) #удаляем имя из сообщения от греха подальше
        user_id = int(user.id)
        asset =  msg_text.split("отправил(а) \U0001fa99")[1].split()[1]
        amount = float(msg_text.split("отправил(а) \U0001fa99")[1].split()[0].replace(',', "")) #получаем сумму ставки надежным образом
    ...
    amounts = float(amount_pattern.group(2).strip()) if amount_pattern else 0.0
    amount = amount
...
def create_keyboard(check=None, summa=None):
    ...
    if check == None and summa == None:
        ...
        claim_check = InlineKeyboardButton(f"🎁 Забрать", url=check)
        bet_button = InlineKeyboardButton("Сделать ставку", url=config.BET_URL)
        kak_button = InlineKeyboardButton(" Как сделать ставку❓", url=config.RULES_LINK)
    ...
    try:
        ...
        if emoji and winning_values or emoji or winning_values:
            ...
            if float(cb_balance) >= float(summa) and float(summa) >= 0.02:
                check = await create_check(summa, us_id)
            ...
            if 'отправил(а)' in message.text:
                try:
                    async with processing_lock:
                        ...
                        parsed_data = parse_message(message.md_text, message)
                                ...
                                if user_id.isdigit():
                                    ...
                                    if not comment_lower or not comment_lower.strip() or comment_lower == '':
                                        ...
                                        if count == 0:
                                            ...
                                            bet_msg = await bot.send_message(config.CHANNEL_ID, f"<b>[💎]Новая ставка</b>\n\n<blockquote><b>Никнейм игрока: {username}</b></blockquote>\n<blockquote><b>Сумма ставки: {round(amount, 3)}$</b></blockquote>\n<blockquote><b>Ставка игрока: {comment_lower}</b></blockquote>")
                                            for bet_type in DICE_CONFIG.keys():
                                                if bet_type == parsed_data['comment']:
